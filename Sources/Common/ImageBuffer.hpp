/**
 * Шаблонный класс буфера изображения
 ...
 * \tparam T Тип или класс описывающий цвет одного элемента (текселя) текстуры
 */
template<typename T>
class ImageBuffer
{
...
public:
     ...
     * \param x Координаты точки по X
     * \param y Координаты точки по Y
     * \param color Цвет (значение)
     * \param safe Проверка на выход за пределы буфера
     */
    void setPoint(unsigned x, unsigned y, T color, bool safe = true)
    {
        if(safe){
